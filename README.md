...
- struct是结构体，结构体里面定义的变量所占的字节大小会根据计算机的规则进行扩充，比如int 会扩充到4比特。而aligns是用于规定struct的起始位置。本质就是为了方便查找

- 指针可以指向nullptr，但是引用必须指向具体数，且引用的底层也是指针实现的。指针用完之后一定要释放，指针定义后可以改变指向，但是引用不能改变。对于函数来说，一般使用引用才能改变值，否则传入的只是值的副本。引用和指针一样都是占用内存的，而且占用的内存一样大；引用一般是用于与栈对象进行绑定，而指针一般是用于堆对象，所以指针用完必须释放，否则会成为悬垂指针。指针的大小一般是随着系统的位数而定，比如我64位的电脑那么指针的大小就是8字节
...
- struct是C语言中的，Class是C++中的。struct的默认属性是公有，class默认属性是私有。struct默认public继承，class默认是private继承。public继承后的属性就变成了public，private继承后的属性就是private，不管什么继承都不能访问private，但是如果是protected继承，那么就会变成protected，此时可以访问protect类

- static的局部变量不会在函数结束的时候销毁，会在程序运行结束后的时候销毁，不会存在栈，而是存在程序运行的数据段。静态全局变量的话会使得只能在文件内使用，可以跨文件可见，通过extern。静态成员变量需要在对象外面定义，类声明仅声明其存在

- final代表了这个类是继承基类的最后的类，再来一次就报错。虚函数存在的意义就是C++中的多态，虚函数是在运行时决定使用函数的哪个版本，允许一个虚函数被多次重写
...
- 函数重载是同一个水平方向方面--同一函数名不同返回值。函数重写（override）只能是对于父类和子类，且重写必须是对虚函数进行重写。隐藏是父类的函数不是虚函数。重载（静态的多态)是编译时进行，重写(动态的多态））是运行时进行。
...
- volatile表示了该变量可能被外界所改变，可以用于多线程中，代表告诉编译器某个变量会被多个线程同时访问并改变。mutable代表了即使设置为const也能改变其值。explicit表示对象不能进行类型的隐式转换
...
- 对象复用就是将对象复制进池子中（放回池子之前必须重置对象状态（避免脏数据）），再次调用的时候就不需要malloc。零拷贝就是避免内存拷贝以提高效率，DMA就是一个硬件协议。
...
- 普通的二叉查找树在插入和删除操作后可能会变得高度不平衡，导致树的高度退化为n，在这种情况下，插入、删除和查找操作的时间复杂度都会退化n； 红黑树通过保持树的平衡，确保了插入、删除和查找操作的时间复杂度为log n
